...
def parse_detail_from_html(page) -> dict:
    ...
    用 JS innerText 取可见文本，自动过滤 BOSS 反爬隐藏 span。
    公司介绍/标签在公司页右侧面板中不存在，保持空值。
    """
    ...
    return {
        "address":        js_text('.job-address-desc'),
        "description":    js_text('.job-detail-box .desc'),
        "boss_active":    js_text('.boss-active-time'),
        "company_intro":  "",
        "company_labels": [],
    }
        ...
        for i, job in enumerate(collected_jobs):
            ...
            try:
                if i == 0:
                    ...
                    page.listen.stop()
                    print(f"   等待详情响应... {'成功' if res else '超时'}")
                    if res:
                        body = res.response.body
                        if isinstance(body, str):
                            body = json.loads(body)
