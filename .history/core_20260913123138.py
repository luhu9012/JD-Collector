...
"""
    ...
    def collect_search_fast(self, page, params: dict, limit: int) -> tuple:
        ...
        while len(collected_jobs) < limit and not self.stop_flag:
            res, page = self._safe_listen_wait(page, 'joblist.json', url, timeout=12)
            if not res:
    ...
    def run(self, params: dict):
        try:
            ...
            page.listen.start('joblist.json')
            page.get(search_url)
            res, page = self._safe_listen_wait(page, 'joblist.json', search_url, timeout=12)
            try:
                page.listen.stop()
            except Exception:
                pass
            ...
            if not res:
                ...
                if not company_card and not cards:
                    ...
                    res, page = self._safe_listen_wait(page, 'joblist.json', search_url, timeout=20)
                    try:
                        page.listen.stop()
                    except Exception:
                        pass
    ...
    def _safe_listen_wait(self, page, pattern: str, url: str = None, timeout: int = 12):
        """Attempt to call `page.listen.wait`. If the listener appears broken
        (internal driver is None), try to recreate the page and restart the
        listener once, then wait again. Returns a tuple (res, page).
        """
        try:
            return page.listen.wait(timeout), page
        except AttributeError:
            # DrissionPage listener internal driver may become None intermittently
            self.log("   监听器异常 (NoneType)，尝试重连页面...", "warn")
            try:
                page = get_page(self.port)
                # If we have a URL, open it while listening for the given pattern
                if url:
