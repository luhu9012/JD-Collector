...
"""
    ...
    def collect_search_fast(self, page, params: dict, limit: int) -> tuple:
        ...
        while len(collected_jobs) < limit and not self.stop_flag:
            ...
            try:
                ...
                self.log(f"   解析响应 body 失败：{e}", "warn")
                try:
            ...
            job_list = body.get('zpData', {}).get('jobList', []) if isinstance(body, dict) else []
            has_more = body.get('zpData', {}).get('hasMore', False) if isinstance(body, dict) else False
            res_count = body.get('zpData', {}).get('resCount', 0) if isinstance(body, dict) else 0
            self.log(f"   解析响应：本批 {len(job_list)} 条，resCount={res_count}")

            if scroll_count == 0:
                self.log(f"✅ 搜索结果：共 {res_count} 个职位")
            self.log(f"   本批获取 {len(job_list)} 条，已累计 {len(collected_jobs)} 条")

            # iterate and append, counting how many are newly added this batch
            new_added = 0
            ...
            self.log(f"   本批新增 {new_added} 条，累计 {len(collected_jobs)} 条")
    ...
    def collect_search_detail(self, page, params: dict, limit: int) -> tuple:
        ...
        while len(collected_jobs) < limit and not self.stop_flag:
            ...
            job_list = body.get('zpData', {}).get('jobList', []) if isinstance(body, dict) else []
            has_more = body.get('zpData', {}).get('hasMore', False) if isinstance(body, dict) else False
            self.log(f"   解析响应：本批 {len(job_list)} 条")
            self.log(f"   累计已收集 {len(collected_jobs)} 条（解析后），has_more={has_more}")

            # iterate and append, with safer per-item handling and batch summary
            new_added = 0
            for idx, item in enumerate(job_list):
                ...
                try:
                    if not isinstance(item, dict):
                        # unexpected item type — log for debugging and skip
                        self.log(f"   跳过：job_list 中第{idx}项类型非 dict，类型={type(item)}", "debug")
                        continue
                    job = parse_job_item(item)
                    job['source_url'] = f"https://www.zhipin.com/job_detail/{job.get('job_id','')}.html"
                    if job.get('job_id') and job.get('job_id') in saved_ids:
                        skipped += 1
                        continue
                    collected_jobs.append(job)
                    new_added += 1
                except Exception as e:
                    self.log(f"   解析单条记录失败：{e} （索引={idx}）", "warn")
                    try:
                        preview = str(item)[:200]
                        self.log(f"   解析失败项预览：{preview}", "debug")
                    except Exception:
                        pass
        ...
        self.log(f"   页面卡片数量：{len(cards)} 个（用于点击获取详情）")
        ...
        for i, job in enumerate(collected_jobs):
            ...
            try:
                if i == 0:
                    ...
                    detail_patterns = [
                        'zpgeek/job/detail',
                        'wapi/zpgeek/job/detail',
                        'zpgeek/job/detail.json',
                    ...
                    # start listeners for detail patterns
                    for dp in detail_patterns:
                        try:
                            ...
                            pass
                    if i < len(cards):
                        self.log(f"   点击卡片索引 {i}（共 {len(cards)} 张卡）以触发详情请求...")
                        try:
                            cards[i].click()
                        except Exception as e:
                            self.log(f"   点击卡片失败：{e}", "warn")
                    else:
                        self.log(f"   卡片索引 {i} 超出页面卡片范围，跳过", "warn")
    ...
    def _safe_listen_wait(self, page, pattern, url: str = None, timeout: int = 12):
                ...
                while waited < wait_timeout:
                    try:
                        ...
                        try:
                            ...
                            cur_url = '<unknown>'
                        self.log(f"   监听模式 '{pat}' 等待中，已等待 {waited}s，页面URL={cur_url}", "debug")
        ...
        self.log(f"   调试：尝试捕获网络响应，候选模式={patterns}，超时={timeout}s", "debug")
        try:
            ...
            if res:
                ...
                self.log(f"   捕获到响应：url={url_matched}  预览={str(preview)[:300]}", "debug")
                return res, page
        ...
        try:
            ...
            if res:
                try:
                    ...
                    url_matched = None
                self.log(f"   重连后捕获到响应：url={url_matched}", "debug")
            else:
