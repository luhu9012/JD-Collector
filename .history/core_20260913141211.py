...
"""
    ...
    def collect_search_detail(self, page, params: dict, limit: int) -> tuple:
        ...
        # 通过点击左侧职位卡片触发详情请求并拦截（严格避免直接构造/请求详情URL）
        if collected_jobs:
            self.log("\n🔎 开始通过点击列表触发并拦截详情响应...")
            detail_patterns = [
            ...
            dom_index = 0
            for idx, job in enumerate(collected_jobs):
                if self.stop_flag:
                    break
                try:
                    # locate candidate cards in DOM
                    cards = page.eles('css:li.job-card-box') or []
                    target_card = None
                    # try to match by job_id in href
                    for c in cards:
                        try:
                            link = c.ele('css:.job-name')
                            href = link.attr('href') if link else ''
                        except Exception:
                            href = ''
                        if job.get('job_id') and href and job.get('job_id') in href:
                            target_card = c
                            break

                    # fallback: use same-index card if available
                    if not target_card and dom_index < len(cards):
                        target_card = cards[dom_index]

                    if not target_card:
                        self.log(f"   ⚠️ 未找到卡片以触发详情：{job.get('job_name')} id={job.get('job_id')}", "warn")
                        continue

                    # start listeners for candidate detail patterns
                    ...
                    # click card to trigger detail request
                    try:
                        target_card.click()
                    except Exception:
                        try:
                            target_card.click(by_js=True)
                        except Exception as e:
                            self.log(f"   点击卡片失败：{e}", "warn")
                            # stop any started listeners and continue
                            try:
                                page.listen.stop()
                            except Exception:
                                pass
                    ...
                    if res:
                        ...
                        # fallback to right-side HTML panel
                        try:
                    ...
                    self.log(f"   点击并获取详情时异常：{e}", "warn")
                finally:
                    # always advance to next DOM card so we don't get stuck on the same one
                    dom_index += 1
                    # if we've exhausted visible cards, try re-querying in next loop (no negative index)
                    if dom_index >= max(len(page.eles('css:li.job-card-box') or []), 1):
                        # allow dom_index to keep increasing; next iteration will refresh `cards`
                        pass
        # 公司名模式：AI过滤
        if params.get('search_type') == 'company' and collected_jobs:
            self.log("\n🤖 AI过滤目标岗位...")
