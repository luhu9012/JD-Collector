...
def parse_detail(body: dict) -> dict:
    ...
    brand_info = body.get("zpData", {}).get("brandComInfo", {})
    boss_info  = body.get("zpData", {}).get("bossInfo", {})
    return {
        "address":        job_info.get("address", ""),
        "description":    job_info.get("postDescription", ""),
        "company_intro":  brand_info.get("introduce", ""),
        "company_labels": brand_info.get("labels", []),
...
"""
    ...
    def _click_and_get_detail(self, page, cards, idx: int) -> dict:
        """
        点击卡片获取详情。
        第0个卡片默认已加载，需先切到第1个再切回来。
        其余卡片直接点击。
        """
        try:
            if idx == 0:
                # 先切到第1个卡片触发请求，消费掉默认加载
                page.listen.start('zpgeek/job/detail')
                cards[1].click()
                page.listen.wait(timeout=8)
                page.listen.stop()
                time.sleep(0.5)
                # 再切回第0个
                page.listen.start('zpgeek/job/detail')
            ...
            if res:
                ...
                self.log(f"   ✅ 详情获取成功")
                return parse_detail(body)
            else:
                self.log(f"   ⚠️  详情超时，保存基础信息", "warn")
                return {}
    ...
    def collect_search_detail(self, page, params: dict, limit: int) -> tuple:
        ...
        for i, job in enumerate(collected_jobs):
            ...
            if i < len(cards):
                detail = self._click_and_get_detail(page, cards, i)
                if detail:
                    job.update(detail)
                time.sleep(random.uniform(1.5, 2.5))
            else:
                self.log(f"   ⚠️  卡片数量不足，跳过详情", "warn")
        ...
        while collected < limit and not self.stop_flag:
            ...
            for idx, card in enumerate(cards):
                ...
                try:
                    ...
                    self.log(f"\n   [{idx+1}/{len(cards)}] {job['job_name']}")

                    # 每次重新获取卡片，避免 DOM 引用失效
                    target_cards = page.eles('css:li.job-card-box')
                    if idx < len(target_cards):
                        detail = self._click_and_get_detail(page, target_cards, idx)
                        if detail:
                            job.update(detail)
                        time.sleep(random.uniform(1.5, 2.5))
