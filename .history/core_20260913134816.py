    ...
    def collect_search_fast(self, page, params: dict, limit: int) -> tuple:
        ...
        while len(collected_jobs) < limit and not self.stop_flag:
            ...
            for item in job_list:
                ...
                # debug: show parsed item fields to diagnose why none are collected
                try:
                    self.log(f"   解析项：job_id={job.get('job_id')} job_name={job.get('job_name')}", "debug")
                except Exception:
                    pass
                job['source_url'] = f"https://www.zhipin.com/job_detail/{job['job_id']}.html"
                if job['job_id'] and job['job_id'] in saved_ids:
                    skipped += 1
                    self.log(f"   跳过（已存在）：job_id={job.get('job_id')}", "debug")
                    continue
                collected_jobs.append(job)
                try:
                    self.log(f"   已加入待采集：job_id={job.get('job_id')}", "debug")
                except Exception:
                    pass
