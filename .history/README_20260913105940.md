...
本项目在 macOS 上运行时需要系统的开发工具链来调用 `xcrun` 等命令。为保持轻依赖，优先安装 Command Line Tools 而不是完整的 Xcode：
...
# 若安装后仍报错，手动切换到 CLT 路径：
sudo xcode-select --switch /Library/Developer/CommandLineTools

# 或者在当前终端会话临时设置（仅当 CLT 已安装时）
...
我已添加一个实用脚本 `scripts/check_dev_tools.sh`，用于检测常见问题并给出修复建议：

```bash
chmod +x scripts/check_dev_tools.sh
