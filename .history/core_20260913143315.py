...
"""
    ...
    def collect_search_detail(self, page, params: dict, limit: int, initial_res=None) -> tuple:
        ...
        while success + failed < limit and not self.stop_flag:
            list_res = pending_list_res or page.listen.wait(15)
            ...
            cards = page.eles('css:li.job-card-box') or page.eles('css:.job-card-box') or []
            self.log(f"📋 当前 DOM 找到 {len(cards)} 张职位卡片，开始逐张点击")

            for job in jobs:
                if self.stop_flag or success + failed >= limit:
                    break
                card_index = job.pop('_card_index', 0)
                ...
                try:
                    ...
                    card = (page.eles('css:li.job-card-box') or page.eles('css:.job-card-box'))[card_index]
                    card.scroll.to_see()
                    ...
                    detail_res = page.listen.wait(8)
                except Exception as e:
                    self.log(f"   ⚠️ 点击或监听失败：{e}", "warn")
                finally:
                    try:
                        page.listen.stop()
                    except Exception:
