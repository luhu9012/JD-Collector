...
"""
    ...
    def collect_search_detail(self, page, params: dict, limit: int) -> tuple:
        ...
        # （已合并）后续将按 AI 过滤后的列表逐个点击并获取详情（每次重新查询 DOM，避免重复点击）
        # 公司名模式：AI过滤
        if params.get('search_type') == 'company' and collected_jobs:
            self.log("\n🤖 AI过滤目标岗位...")
            collected_jobs = self.ai_filter(collected_jobs)

        # 逐个点击卡片拿详情（重新查询 DOM，尝试按 job_id 匹配 card，找不到则使用下一个可见卡片）
        detail_patterns = [
            'zpgeek/job/detail', 'wapi/zpgeek/job/detail',
            'zpgeek/job/detail.json', 'wapi/zpgeek/job/detail.json',
            'job/detail', 'job/detail.json', 'detail', 'detail.json'
        ]
        self.progress(0, len(collected_jobs))
        dom_index = 0
        ...
        for i, job in enumerate(collected_jobs):
            ...
            self.log(f"\n── [{i+1}/{len(collected_jobs)}] {job['job_name']} ──")
                try:
                    # fresh query of visible cards each iteration
                    cards = page.eles('css:li.job-card-box') or []
                    # debug: capture preview of first few cards (hrefs/text) via JS
                    try:
                        preview_js = (
                            "Array.from(document.querySelectorAll('li.job-card-box')).slice(0,12)"
                            ".map((c,i)=>{var a=c.querySelector('.job-name a')||c.querySelector('.job-name');"
                            "return {idx:i, href: a ? (a.getAttribute('href')||a.innerText.slice(0,120)) : '', html: c.outerHTML.slice(0,200)};})"
                        )
                        cards_preview = page.run_js('JSON.stringify(' + preview_js + ')')
                        self.log(f"   DOM cards preview: {cards_preview}", "debug")
                    except Exception:
                        cards_preview = None

                    target_idx = None
                    # try to match by job_id in preview hrefs if available
                    job_id = job.get('job_id') or ''
                    if job_id and cards_preview:
                        try:
                            import json as _json
                            cp = _json.loads(cards_preview)
                            for it in cp:
                                if job_id and it.get('href') and job_id in it.get('href'):
                                    target_idx = it.get('idx')
                                    break
                        except Exception:
                            target_idx = None

                    # fallback to next visible card index
                    if target_idx is None:
                        if dom_index < len(cards):
                            target_idx = dom_index

                    if target_idx is None:
                        self.log(f"   ⚠️ 未找到卡片以触发详情：{job.get('job_name')} id={job.get('job_id')}", "warn")
                        # try HTML panel as last resort
                        try:
                            job.update(parse_detail_from_html(page))
                            self.log(f"   ⚠️ 使用HTML解析（未找到卡片）")
                        except Exception as e:
                            self.log(f"   HTML解析失败：{e}", "warn")
                    else:
                        # start listeners
                        for dp in detail_patterns:
                            try:
                                page.listen.start(dp)
                            except Exception:
                                pass

                        # click via JS on the specific card index and log result
                        try:
                            js_click = f"(function(){{var cards=document.querySelectorAll('li.job-card-box');var c=cards[{target_idx}];if(!c) return 'no-card';var a=c.querySelector('.job-name a')||c.querySelector('.job-name');if(a){a.click();return 'clicked-link';}else{c.click();return 'clicked-card';}}})()"
                            click_result = page.run_js(js_click)
                            self.log(f"   点击动作结果：index={target_idx} result={click_result}", "debug")
                        except Exception as e:
                            self.log(f"   JS点击失败，尝试 element.click：{e}", "warn")
                            try:
                                # fallback to element click if available
                                if target_idx < len(cards):
                                    cards[target_idx].click()
                            except Exception as e2:
                                self.log(f"   点击卡片失败：{e2}", "warn")
                                try:
                                    page.listen.stop()
                                except Exception:
                                    pass
                        ...
                        res, page = self._safe_listen_wait(page, detail_patterns, None, timeout=10)
                    try:
                        page.listen.stop()
                    except Exception:
                        pass
                    ...
                    if res:
                        ...
                        try:
                            ...
                            self.log(f"   ✅ 详情获取成功（JSON）：{job.get('job_name')}", "success")
                        except Exception as e:
                            self.log(f"   解析详情失败：{e}", "warn")
                    else:
                        try:
                            job.update(parse_detail_from_html(page))
                            self.log(f"   ⚠️ JSON超时，已降级为HTML解析：{job.get('job_name')}", "warn")
                        except Exception as e:
                            self.log(f"   HTML解析详情失败：{e}", "warn")
