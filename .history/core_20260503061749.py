...
"""
    ...
    def collect_search_detail(self, page, params: dict, limit: int) -> tuple:
        ...
        for i, job in enumerate(collected_jobs):
            ...
            if i < len(cards):
                try:
                    # 先点其他卡片让详情切换走，保证目标卡片点击必然触发请求
                    other_index = 1 if i == 0 else 0
                    if other_index < len(cards):
                        cards[other_index].click()
                        time.sleep(0.8)
    ...
    def collect_company_detail(self, page, company_url: str,
        ...
        # 先点第2个卡片，消除页面默认高亮第1个的状态
        self.log("   初始化：消除默认高亮...")
        init_cards = page.eles('css:li.job-card-box')
        if len(init_cards) >= 2:
            page.listen.start('zpgeek/job/detail')
            init_cards[1].click()
            page.listen.wait(timeout=5)
            page.listen.stop()
            time.sleep(1)
            ...
            for idx, card in enumerate(cards):
                ...
                try:
                    ...
                    if idx < len(target_cards):
                        try:
                            # 先切到其他卡片
                            other_index = 1 if idx == 0 else 0
                            if other_index < len(target_cards):
                                page.listen.start('zpgeek/job/detail')
                                target_cards[other_index].click()
                                page.listen.wait(timeout=5)
                                page.listen.stop()
                                time.sleep(0.5)
